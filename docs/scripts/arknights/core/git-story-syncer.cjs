...
async function cloneStoryDirsOnly(repoUrl, localPath, storyPaths) {
    ...
    if (repoExists) {
        console.log(
            `Updating existing sparse repository: ${path.basename(localPath)}`
        );
        try {
            ...
            console.warn(
                `WARNING: Pull failed, will re-clone: ${error.message}`
            );
            deleteDir(localPath);
        }
    }
    ...
    console.log(
        `Cloning repository with sparse checkout (story dirs only): ${repoUrl}`
    );
 ...
 */
async function syncLanguageStories(
    langCode,
    dryRun = false,
    repoExists = false
) {
    ...
    const targetStoryPath = path.join(
        PROJECT_CONFIG.getDataPath(langCode, "arknights"),
        "story"
    );
    ...
    const success = await extractStoryFiles(
        langCode,
        sourceStoryPath,
        targetStoryPath
    );
    ...
    console.log(
        `${langCode} stories synced: ${afterCount} files (was ${beforeCount})`
    );
...
async function syncAllStories(options = {}) {
    ...
    try {
        ...
        if (!dryRun) {
            ...
            console.log(
                `Sparse checkout repository (story dirs only): ${repoConfig.url}`
            );
            repoCloned = await cloneStoryDirsOnly(
                repoConfig.url,
                ARKNIGHTS_REPO_PATH,
                storyPaths
            );
            ...
            if (!repoCloned) {
                console.error(
                    "ERROR: Failed to sparse checkout repository, aborting..."
                );
                return Object.fromEntries(
                    supportedLanguages.map((lang) => [lang, false])
                );
            }
        }

        for (const langCode of supportedLanguages) {
            results[langCode] = await syncLanguageStories(
                langCode,
                dryRun,
                repoCloned
            );
        ...
        console.log(
            `Success: ${successCount}/${supportedLanguages.length} languages`
        );
    } finally {
        if (cleanup && !dryRun) {
