...
const EXCLUDE_IDS = new Set(["char_1012_skadi2_1", "$ill_amiya_normal"]);
...
function processCharacters() {
    ...
    for (const [id, char] of Object.entries(data)) {
        ...
        const normalizedChar = {
            ...
            validVariants: (char.validVariants || []).map((v) =>
                normalizeId(v)
            ),
        };
...
function processNames() {
    for (const dir of LANG_DIRS) {
        ...
        fs.writeFileSync(filePath, JSON.stringify(newData, null, 2), "utf8");
        console.log(
            `Normalized names.json in ${path.basename(path.dirname(dir))}`
        );
    }
}
...
function processStoryFiles() {
    ...
    for (const [id, stories] of Object.entries(data)) {
        ...
        if (newData[normalizedId]) {
            newData[normalizedId] = Array.from(
                new Set([...newData[normalizedId], ...stories])
            );
        } else {
