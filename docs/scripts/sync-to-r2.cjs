...
async function main() {
    ...
    const characterData = characterEntries
        .map(([id, data]) => {
            const variants =
                data.validVariants && data.validVariants.length > 0
                    ? data.validVariants
                    : data.charType === "operator"
                    ? [`${id}#1$1`]
                    : [];
    ...
    if (characterData.length === 0) {
        console.log(
            "WARNING: No characters with valid variants found for R2 sync"
        );
        return;
    }
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        console.log(
            `Batch ${i + 1} complete: ${result.processed} variants processed, ${
                result.uploaded
            } uploaded, ${result.skipped} skipped`
        );
