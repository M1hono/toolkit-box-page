...
/**
 * Filter out already processed operators
 */
function filterUnprocessedOperators(operators, uploadedFiles) {
    const unprocessed = [];
 ...
 */
async function main() {
    console.log("Starting story avatar sync with smart filtering...\n");
    ...
    const { unprocessed, processed } = filterUnprocessedOperators(operators, uploadedFiles);
    
    console.log(`Total story avatars: ${operators.length}`);
    ...
    if (unprocessed.length === 0) {
        console.log("🎉 All story avatars are already processed!");
        return;
    }
    ...
    for (let i = 0; i < unprocessed.length; i += BATCH_SIZE) {
        ...
        if (totalUploaded >= MAX_UPLOADS_PER_RUN) {
            ...
            console.log(`Remaining: ${unprocessed.length - i} avatars`);
            break;
        }
