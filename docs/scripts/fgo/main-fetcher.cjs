...
} = require("./core/servant-processor.cjs");
...
function loadTranslationFiles(zhPath) {
    ...
    return {
        translations: fs.existsSync(translationsFile)
            ? JSON.parse(fs.readFileSync(translationsFile, "utf8"))
            : {},
        noTranslations: fs.existsSync(noTranslationFile)
            ? JSON.parse(fs.readFileSync(noTranslationFile, "utf8"))
            : {},
        nameMapping: fs.existsSync(nameMappingFile)
            ? JSON.parse(fs.readFileSync(nameMappingFile, "utf8"))
            : SERVANT_TRANSLATION_OVERRIDES,
    };
}
...
async function syncPlatform(platform) {
    ...
    try {
        ...
        console.log(
            `📥 Fetched ${rawServants.length} servants for ${platform}`
        );
        ...
        let translationData = {
            translations: {},
            noTranslations: {},
            nameMapping: SERVANT_TRANSLATION_OVERRIDES,
        };

        if (langCode === "zh_CN" && platform === "CN") {
            translationData = loadTranslationFiles(outputPath);
            finalServants = applyChineseTranslations(
                processedServants,
                translationData
            );
        ...
        const searchIndex = createSearchIndex(
            ...
            translationData.noTranslations
        );
        ...
        console.log(
            ` ${platform} (${langCode}): ${
                Object.keys(finalServants).length
            } servants processed`
        );
        console.log(
