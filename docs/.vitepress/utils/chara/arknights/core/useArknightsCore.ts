...
import type { CharacterData, CharacterNames, UnifiedCharacterData } from '../types';
...
export function useArknightsCore() {
    ...
    const unifiedCharacters = computed<UnifiedCharacterData[]>(() => {
        return Object.keys(allCharacters.value).map(id => {
    ...
    const selectedCharacter = computed(() => 
        selectedCharacterId.value ? unifiedCharacters.value.find(c => c.charId === selectedCharacterId.value) : null
    );
    ...
    async function loadData() {
        try {
            ...
            const globalRes = await fetch('/data/global/arknights/characters.json');
            if (!globalRes.ok) throw new Error('Failed to load global characters');
            allCharacters.value = await globalRes.json();
            
            // Load language-specific names
            const namesRes = await fetch(`/data/${currentLangCode.value}/arknights/names.json`);
            if (!namesRes.ok) throw new Error(`Failed to load names for ${currentLangCode.value}`);
