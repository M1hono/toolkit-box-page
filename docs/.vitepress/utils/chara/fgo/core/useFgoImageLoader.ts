...
export function useFgoImageLoader() {
    ...
    async function loadTranslations() {
        try {
            ...
            try {
                const translationsRes = await fetch(`/data/${langCode}/fgo/translations.json`);
                if (translationsRes.ok && translationsRes.headers.get('content-type')?.includes('application/json')) {
            ...
            try {
                const noTranslationsRes = await fetch(`/data/${langCode}/fgo/no_translation.json`);
                if (noTranslationsRes.ok && noTranslationsRes.headers.get('content-type')?.includes('application/json')) {
    ...
    async function fetchCharacterData() {
        try {
            ...
            try {
                const response = await fetch(`/data/${langCode}/fgo/servants.json`);
                if (response.ok && response.headers.get('content-type')?.includes('application/json')) {
                    const data = await response.json();
                    characterData.value = Object.values(data);
                    console.log("✅ Using local FGO data for", langCode, "- characters:", characterData.value.length);
                    return;
                }
            } catch (localError) {
            ...
            try {
                ...
                if (response.ok && response.headers.get('content-type')?.includes('application/json')) {
                    const data = await response.json();
                    characterData.value = Object.values(data);
                    console.log("✅ Using global FGO data - characters:", characterData.value.length);
                    return;
                }
            } catch (globalError) {
            ...
            console.log("✅ Using remote FGO data, characters:", characterData.value.length);
        } catch (error) {
    ...
    function getCharacterName(char: FgoCharacter): string {
        return translations.value[char.name] ||
    ...
    function extractImageUrls(character: FgoCharacter): Record<string, string> {
        if (character.imageData?.charaFigures) {
            ...
            for (const [category, categoryData] of Object.entries(character.imageData.charaFigures as any)) {
                if (!categoryData || typeof categoryData !== "object") continue;

                for (const [stage, stageData] of Object.entries(categoryData as any)) {
                    ...
                    if (stageData && typeof stageData === "object" && typeof (stageData as any).url === "string") {
