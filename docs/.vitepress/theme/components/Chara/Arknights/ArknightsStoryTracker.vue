        ...
        <div v-if="selectedCharacter" class="content-section">
            <v-row>
                <v-col cols="12" md="4">
                    <v-card>
                        <v-card-text
                            v-if="availableCharacterIds.length > 1"
                            class="pa-3"
                        >
                            <v-select
                        ...
                        <v-card-text class="pa-3 flex-spacer">
                            <div class="variants-grid">
                                <v-chip
                                    v-for="(
                                        variant, idx
                                    ) in selectedCharacter.validVariants"
                                    :key="variant"
                                    :variant="
                ...
                <v-col cols="12" md="8">
                    <v-card>
                        ...
                        >
                            <span>{{
                                t.storyAppearances.replace(
                                    "{count}",
                                    String(characterStories.length)
                                )
                            }}</span>
                        </v-card-title>
...
<script setup lang="ts">
    ...
    function getFullImageUrl(variant: string): string {
        return `https://raw.githubusercontent.com/akgcc/arkdata/main/assets/avg/characters/${encodeURIComponent(
            variant.toLowerCase()
        )}.png`;
    }
...
Available characters for "${name}":`,
    ...
    function openStory(story: any) {
        const url =
            selectedReader.value === "textReader"
                ? story.urls.textReader
                : story.urls.akgcc;
